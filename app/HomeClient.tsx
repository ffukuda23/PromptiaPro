...
//
// A contagem de prompts por categoria (`counts`) vem do Server Component pai
// (app/page.tsx), que lê a view `prompt_counts` do Supabase com ISR. Cada card
// usa `counts[cat.name]` quando disponível e cai no valor fixo `cat.count` como
// fallback (build sem acesso ao banco, Supabase fora do ar, etc.).

import Link from 'next/link'

const categories = [
  { icon: '💰', name: 'Financeiro Pessoal', count: 24, hot: false, isNew: false },
  { icon: '🏪', name: 'Empresa Varejo', count: 16, hot: true, isNew: false },
  { icon: '🏭', name: 'Empresa Atacado', count: 16, hot: false, isNew: false },
  { icon: '⚙️', name: 'Empresa Indústria', count: 16, hot: false, isNew: false },
  { icon: '🛠️', name: 'Empresa Serviços', count: 16, hot: false, isNew: false },
  { icon: '⚖️', name: 'Jurídico', count: 18, hot: false, isNew: false },
  { icon: '📊', name: 'Resumo Econômico', count: 16, hot: false, isNew: false },
  { icon: '🏥', name: 'Laudos Médicos', count: 18, hot: false, isNew: false },
  { icon: '🛒', name: 'Vendas E-commerce', count: 18, hot: true, isNew: false },
  { icon: '👔', name: 'Carreira', count: 18, hot: false, isNew: true },
  { icon: '👥', name: 'RH Empresarial', count: 15, hot: false, isNew: false },
  { icon: '📱', name: 'Marketing Digital', count: 18, hot: true, isNew: false },
  { icon: '🎓', name: 'Estudantes Universitários', count: 24, hot: false, isNew: false },
  { icon: '🚗', name: 'Compra e Venda de Automóveis', count: 16, hot: false, isNew: false },
  { icon: '🏠', name: 'Locação de Imóveis', count: 18, hot: false, isNew: false },
  { icon: '📣', name: 'Marketing Empresarial', count: 20, hot: false, isNew: false },
  { icon: '🎨', name: 'Personalização de Produtos', count: 16, hot: false, isNew: true },
  { icon: '🧘', name: 'Saúde e Qualidade de Vida', count: 18, hot: false, isNew: true },
  { icon: '🎉', name: 'Festas e Eventos', count: 12, hot: false, isNew: false },
  { icon: '🌍', name: 'Novos Idiomas', count: 8, hot: false, isNew: false },
  { icon: '🗂️', name: 'Secretária Particular', count: 18, hot: false, isNew: true },
]
const prompts = [
...
export default function HomeClient({ counts = {} }: { counts?: Record<string, number> }) {
  return (
    <main className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <section id="categorias" className="max-w-5xl mx-auto px-6 pt-4 pb-14">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {categories.map(cat => {
            const n = counts[cat.name] ?? cat.count
            return (
              <Link
                key={cat.name}
                href="/auth/register"
              ...
              >
                ...
                <div className="text-xs" style={{ color: 'var(--muted)' }}>{n} {n === 1 ? 'prompt' : 'prompts'}</div>
              </Link>
            )
          })}
        </div>
